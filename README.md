...
**The record corpus.** `tests/fixtures/records/<root>/` mirrors SkyrimNet-Core's `tests/test_data/records/` byte for byte (the two trees have the same git tree id): `valid/` holds the design's record shapes, `base/` the shipped base content verbatim, `invalid/` the files the validator refuses — each opens with `# refuse: <CODE>` and is refused with exactly that code; a second line `# engine: loads` marks a file the engine still loads, refused here to catch an authoring mistake such as `enabled:` on an item or spell — and `engine_invalid/` the refusals only the engine makes, which the validator accepts. `tests/record-corpus.test.mjs` runs the tree; a rule change updates both copies in one change.
