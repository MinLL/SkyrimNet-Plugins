...
- **Must (bundles only):** `changelog` says what changed in this version, for the user deciding
  ...
  shows them as a history, so do not paste previous entries into it. Listings have no versions
  and carry no changelog.
