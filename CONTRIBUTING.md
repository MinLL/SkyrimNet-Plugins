...
- **`enabled:` means the user's on/off toggle** wherever a record carries it. Spell and item records say whether the form appears in NPC equipment and spell lists in prompts with `show_in_prompts: true|false` (true when omitted); a spell or item file carrying `enabled` is rejected.
